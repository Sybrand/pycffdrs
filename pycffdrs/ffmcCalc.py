...
def ffmcCalc(ffmc_yda, temp, rh, ws, prec):
    """
    ...
    wmo = np.where(prec > 0.5,
                   np.where(wmo > 150, wmo + 0.0015 * (wmo - 150) * (wmo - 150) *
    ...
    #  termperature of 21.1 C
    z = np.where(wmo > ed, 0.424 * (1 - (rh/100) ** 1.7) + 0.0694 * sqrt(ws) *
                 (1 - (rh/100) ** 8), z)
