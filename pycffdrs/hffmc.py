...
"""
from warnings import showwarning as warning
from numpy import ndarray
import numpy as np


def hffmc(weatherstream: ndarray, ffmc_old=85, time_step=1,
          calc_step=False, batch=True, hourlyFWI=False):
    #############################################################################
    ...
    #############################################################################
    t0 = time_step
    weatherstream.dtype.names = [name.lower() for name in weatherstream.dtype.names]
    # set up number of stations
    if (batch):
        if "id" in weatherstream.dtype.names:
            n = len(np.unique(weatherstream['id']))
            if len(np.unique(weatherstream["id"][1:n])) != n:
                raise "Multiple stations have to start and end at the same dates/time, and the data must be sorted by date/time and id"
        else:
            n = 1
    else:
        n = len(weatherstream)

    if len(ffmc_old) == 1 and n > 1:
        Fo = np.repeat(ffmc_old, n)
    else:
        Fo = ffmc_old

    # set some local scope variables
    if "temp" in weatherstream.dtype.names:
        Tp = weatherstream["temp"]
    else:
        Tp = None
    if "rh" in weatherstream.dtype.names:
        H = weatherstream["rh"]
    else:
        H = None
    if "ws" in weatherstream.dtype.names:
        W = weatherstream["ws"]
    else:
        W = None
    if "prec" in weatherstream.dtype.names:
    ...
    if calc_step:
        if "hr" in weatherstream.dtype.names:
            hr = weatherstream["hr"]
        else:
            hr = None
    if hr is None:
