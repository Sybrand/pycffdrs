...
def get_thing(FUELTYPE, a):
    return a[FUELTYPE]
    # return -9

def ROScalc(FUELTYPE, ISI):
    ...
    RSI = np.empty(len(ISI))
    RSI.fill(-1)
    # TODO: trying to figure this out - the np.isin part works, but how
    # do I apply 
    # Is the answer to apply a map and get all the values I want???
    RSI = np.where(np.isin(FUELTYPE, ["C1", "C2", "C3", "C4", "C5", "C7", "D1", "S1", 
                                    "S2", "S3"]), get_thing(FUELTYPE, a) , RSI )
    return RSI
    # np.where(np.isin(["a", "b", "c"], ["a", "b"]), b, "z")
